<?php
/*
 * Collection of internal library
 */
...
function wpws_get_content($url = '', $selector = '', $wpwsopt = '') {
	...
	if ($cache_status) {
		...
		if ($scrap['key']) {
			...
			if ($wpwsopt['on_error'] == 'wpws_error_hide') return $header.$footer;
			if ($wpwsopt['on_error'] == 'wpws_error_show') return $header.$scrap['value'].$footer;
			if ($wpwsopt['on_error'] == 'wpws_error_show_cache') {
				if ($cache_value['data'])
					return wpws_parse_byselector($cache_value['data'], $url, $selector, $wpwsopt); 
				if (!$cache_value['data'])
					return $header.$scrap['value'].$footer;
			} 
			else { return $wpwsopt['on_error']; } 
		}
	}
}
...
function wpws_shortcode($atts) {
	...
	return wpws_get_content($url, $selector, 'postargs='.$postargs.'&cache='.$cache.'&user_agent='.$user_agent.'&timeout='.$timeout.'&on_error='.$on_error.'&output='.$output.'&clear_regex='.$clear_regex.'&replace_regex='.$replace_regex.'&replace_with='.$replace_with.'&basehref='.$basehref.'&striptags='.$striptags.'&htmldecode='.$htmldecode);
}
